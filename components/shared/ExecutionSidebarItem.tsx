...
export const ExecutionSidebarItem = ({ isCollapsed }: Props) => {
  ...
  const projectId = params.projectId as string || 'suma-os'; // Fallback por seguridad

  const isActive = pathname.includes('/execution');
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className={cn(
        ...
        <Link 
          href={`/${projectId}/execution/kanban`} 
      ...
      {!isCollapsed && isOpen && (
        <div className="pl-4 ml-2 border-l border-zinc-800 space-y-1 mt-1 animate-in slide-in-from-top-1 duration-200">
          <SubItem href={`/${projectId}/execution/kanban`} label="Kanban" icon={<Kanban size={14}/>} currentPath={pathname} />
          <SubItem href={`/${projectId}/execution/backlog`} label="Backlog" icon={<ListTodo size={14}/>} currentPath={pathname} />
          <SubItem href={`/${projectId}/execution/sprints`} label="Sprints" icon={<RefreshCw size={14}/>} currentPath={pathname} />
        </div>
      )}
    </div>
  );
...
const SubItem = ({ href, label, currentPath, icon }: any) => {
  ...
  return (
    <Link href={href} className={cn("flex items-center gap-2 py-1.5 px-3 text-xs rounded-md transition-colors", isSelected ? "text-blue-400 bg-blue-500/10" : "text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50")}>
      {icon}
      {label}
    </Link>
  );
